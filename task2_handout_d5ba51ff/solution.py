...
class Model(object):
    ...
    def __init__(self):
        ...
        self.num_epochs = 45  # number of training epochs
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    # Perform forward pass
                    current_logits, current_log_prior, current_log_variational_posterior = self.network(batch_x)            

                    #Calculate the loss
                    #loss = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
                    loss = (current_log_variational_posterior - current_log_prior) + F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.use_bias = bias
        self.mu_weight = torch.nn.Parameter(torch.zeros((out_features, in_features)), requires_grad=True)
        self.rho_weight = torch.nn.Parameter(torch.ones((out_features, in_features)), requires_grad=True)
        self.mu_weight.data.normal_(0.0, 0.1) # try different values here
        self.rho_weight.data.normal_(-5, 0.1) # try different values here
        self.mu_bias = torch.nn.Parameter(torch.zeros(out_features, requires_grad=True))
        self.rho_bias = torch.nn.Parameter(torch.ones(out_features, requires_grad=True))
        self.mu_bias.data.normal_(0.0, 0.1) # try different values here
        self.rho_bias.data.normal_(-5, 0.1) # try different values here
        self.epsilon_weight = torch.zeros((out_features, in_features))
        self.epsilon_bias = torch.zeros(out_features)
        ...
        self.prior = UnivariateGaussian(torch.tensor(0.0), torch.tensor(2.0))
                                                        #HYPERPARAMETER 
        ...
        # self.w_mu = torch.nn.Parameter(torch.zeros((out_features, in_features)))
        # self.w_rho= torch.nn.Parameter(torch.sqrt(torch.tensor(2/self.in_features))*torch.ones((out_features, in_features)))

        # self.weights_var_posterior = MultivariateDiagonalGaussian(
        #       self.w_mu,                                                  #HYPERPARAMETER 
        #       self.w_rho
        #    )
        
        self.weights_var_posterior = MultivariateDiagonalGaussian(
              self.mu_weight,                                                  #HYPERPARAMETER 
              self.rho_weight)
        ...
        if self.use_bias:
            ...
            # self.b_rho= torch.nn.Parameter(torch.sqrt(torch.tensor(2/self.in_features))*torch.ones(out_features))
            # self.bias_var_posterior = MultivariateDiagonalGaussian(
            #   self.b_mu,                                                  #HYPERPARAMETER 
            #   self.b_rho
            # )
            
            self.bias_var_posterior = MultivariateDiagonalGaussian(
              self.mu_bias,                                                  #HYPERPARAMETER 
              self.rho_bias)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        self.epsilon_weight = torch.normal(torch.zeros(self.out_features, self.in_features), torch.ones(self.out_features, self.in_features))
        weights = self.mu_weight + \
                torch.log(1+torch.exp(self.rho_weight))*self.epsilon_weight


        if self.use_bias:
            self.epsilon_bias = torch.normal(torch.zeros(self.out_features), torch.ones(self.out_features))
            bias = self.mu_bias + torch.log(1+torch.exp(self.rho_bias))*self.epsilon_bias
            log_variational_posterior = self.weights_var_posterior.log_likelihood(weights) + self.bias_var_posterior.log_likelihood(bias)
            log_prior = torch.sum(self.prior.log_likelihood(weights)) + torch.sum(self.prior.log_likelihood(bias))
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        log_prior_local =  torch.tensor(0.0)
        log_variational_posterior_local = torch.tensor(0.0)
        ...
        for idx, current_layer in enumerate(self.layers):
            ...
            log_prior_local += new_log_prior
            log_variational_posterior_local += new_log_variational_posterior
            if idx < len(self.layers) - 1:
                new_features = self.activation(new_features)
            output_features = new_features

        log_prior = log_prior_local
        log_variational_posterior = log_variational_posterior_local
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        self.mu = mu
        self.rho = rho
        self.dist = torch.distributions.normal.Normal(self.mu, torch.log(1+torch.exp(self.rho)))      #torch.log(1.0+torch.exp(self.rho)))   # self.rho)                   #torch.log(1.0+torch.exp(self.rho)))
        
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        return torch.sum(self.dist.log_prob(values))
