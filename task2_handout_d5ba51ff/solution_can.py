...
class Model(object):
    ...
    def __init__(self):
        ...
        learning_rate = 8e-4  # training learning rates
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    current_logits, log_p, log_var_p = self.network(batch_x) 
                    #KL_divergance = (log_var_p * (log_var_p / log_p).log()).sum()
                    loss = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum') + (log_var_p - log_p) # + KL_divergence
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.epsilon = torch.zeros((out_features, in_features))
        
        # self.epsilon = torch.normal(0.0, 1.0, size=(self.out_features, self.in_features))
        self.mu = torch.nn.Parameter(torch.zeros(out_features * in_features), requires_grad=True)
        self.rho = torch.nn.Parameter(torch.ones(out_features * in_features), requires_grad=True)
        self.mu.data.normal_(0.0, 0.2) # try different values here
        self.rho.data.normal_(-5, 0.2) # try different values here
        ...
        self.prior = UnivariateGaussian(torch.tensor(0.0),torch.tensor(3.0)) # Not sure if it shoud be uni or multi-variate Gaussian
        #self.prior = MultivariateDiagonalGaussian(
        #    torch.tensor(torch.zeros((out_features, in_features))), 
        #    torch.tensor(torch.ones(out_features, in_features))
        #)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        #epsilon = torch.normal(0.0, 1.0, size=(self.out_features, self.in_features))
        self.epsilon.data.normal_(0.0, 3.0)
        
        weights = self.mu.reshape((self.out_features, self.in_features)) + \
                torch.log(1+torch.exp(self.rho.reshape(self.out_features, self.in_features)))*self.epsilon
        log_variational_posterior = torch.sum(self.weights_var_posterior.log_likelihood(torch.flatten(weights)))
        log_prior = torch.sum(self.prior.log_likelihood(torch.flatten(weights)))
        
        
        bias = None
        if self.use_bias:
            self.epsilon_bias.data.normal_(0.0, 3.0)
            bias = self.mu_bias.reshape(self.out_features) + torch.log(1+torch.exp(self.rho_bias.reshape(self.out_features)))*self.epsilon_bias
            log_variational_posterior += torch.sum(self.bias_var_posterior.log_likelihood(bias))
            log_prior += torch.sum(self.prior.log_likelihood(bias))
...
class BayesNet(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: typing.Tuple[int, ...], out_features: int):
        ...
        self.dropout = nn.Dropout(0.5) # play with this number, not sure if the layer is ON during inference too
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        current_features = x
        # copy-paste from dense layer with minor modifications    
        for idx, current_layer in enumerate(self.layers):
            current_features, log_p, log_var_p = current_layer(current_features)
            if idx != len(self.layers) - 1:
                current_features = self.activation(current_features)
            log_prior += log_p
            log_variational_posterior += log_var_p
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        self.mu = mu
        self.rho = rho
        self.dist = torch.distributions.normal.Normal(self.mu, torch.log(1+torch.exp(self.rho)))
