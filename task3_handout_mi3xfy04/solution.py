...
class BO_algo(object):
    def __init__(self):
        ...
        # TODO: enter your code here, RADNOMLY INITIALIZE
        #x0 = 6*np.random.rand(1,1)[0]
        #x1 = 6*np.random.rand(1,1)[0]
        init_pt = 6*np.random.rand(1,2)
        #init_pt = np.array([[2,2]])
        #print(init_pt)
        self.previous_points = [ [init_pt[0,0], init_pt[0,1], f(init_pt)[0], c(init_pt)[0]] ]
        ...
        self.objective_model = sklearn.gaussian_process.GaussianProcessRegressor(kernel = ConstantKernel(1.5, constant_value_bounds="fixed") * RBF(1.5, length_scale_bounds="fixed"), alpha = (1e-2))  
        # TODO : GP model for the constraint function
        self.constraint_model = sklearn.gaussian_process.GaussianProcessRegressor(kernel = ConstantKernel(3.5, constant_value_bounds="fixed") * RBF(2.0, length_scale_bounds="fixed"), alpha = (5e-3))  
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: enter your code here
        print('previous points',self.previous_points)
        #print('sliced:',np.self.previous_points[2])
        #print('X:',x)
        mean_obj, std_obj = self.objective_model.predict(x.reshape(1,-1), return_std = True)
        mean_constr, std_constr = self.constraint_model.predict(x.reshape(1,-1), return_std = True)
        print('COVARIANCE:',std_constr**2)
        #print('MEAN:', mean_obj)
        min_index = np.argmin(np.array(self.previous_points)[:,2])
        t = np.array(self.previous_points)[min_index,2]
        z_x = (t - mean_obj)/std_obj
        #print('ZX:',z_x)
        cdf_eval = scipy.stats.norm.cdf(z_x,0, 1)
        pdf_eval = scipy.stats.norm.pdf(z_x, 0, 1)
        constraint_prob = scipy.stats.norm.cdf(0, mean_constr, std_constr**2)
        EI_x = std_obj * (z_x * cdf_eval + pdf_eval)
        #print('t',t)
        acquisition_function = EI_x * constraint_prob
        #print('ACQUISITION FUNCTION:',acquisition_function)
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        # TODO: enter your code here
        self.objective_model.fit(np.array(self.previous_points)[:,0:2],np.array(self.previous_points)[:,2])
        self.constraint_model.fit(np.array(self.previous_points)[:,0:2],np.array(self.previous_points)[:,3])
    ...
    def get_solution(self) -> np.ndarray:
        ...
        # TODO: enter your code here, BEST SOLN SO FAR
        valid_pts = (np.array(self.previous_points)[:,3]<=0)
        if np.sum(valid_pts > 0):
            valid_pts_arr = np.array(self.previous_points)[valid_pts,:]
            min_index = np.argmin(valid_pts_arr[:,2])
            return valid_pts_arr[min_index,0:2]
        else:
            min_index = np.argmin(np.array(self.previous_points)[:,2])
            return np.array(self.previous_points)[min_index,0:2]
...
CONSTRAINT_OFFSET = - 1.8  # This is an offset you can change to make the constraint more or less difficult to fulfill
