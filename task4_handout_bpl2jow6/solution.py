...
class MLPCategoricalActor(nn.Module):
    ...
    def _distribution(self, obs):
        """Takes the observation and outputs a distribution over actions."""        #SINCE RANDOMIZED POLICIES
...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            pi, log_probs = self.pi.forward(state)
            action = pi.sample()
            log_prob = self.pi._log_prob_from_distribution(pi,action)
            value_estimate_action = self.v.forward(state)
        ...
        return action, value_estimate_action, log_prob
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        shifted_v_values = np.roll(vals,shift = 1)
        residuals = self.gamma * shifted_v_values - vals + rews
        self.phi_buf[path_slice] = discount_cumsum(residuals[:-1:], self.gamma * self.lam)
        ...
        #print(rews.shape)
        #print(path_slice)
        gamma_to_the_t = [self.gamma ** t for t in range(rews.shape[0]-1)]
        self.ret_buf[path_slice] = discount_cumsum(rews[:-1:], self.gamma) * gamma_to_the_t 
...
class Agent:
    ...
    def pi_update(self, data):
        ...
        #returns = torch.tensor(np.unique(ret), requires_grad=True)
        #loss = torch.mean(returns)

        policy = self.ac.pi._distribution(obs)            
        log_probs = policy.log_prob(act)  
        #loss = - torch.sum(log_probs * ret)        #FOR REINFORCE WITH REWARDS TO GO!!!!!
        loss = - torch.sum(log_probs * phi)
        #print(results)
    ...
    def v_update(self, data):
        ...
        for i in range(100):
            ...
            v_theta = self.ac.v.forward(obs[i])
            with torch.no_grad():
                v_bootstrapped = self.ac.step(obs[i+1])[1]
                r = rew[i+1]
            bellman_loss = 0.5*(v_theta - r - v_bootstrapped)**2
    ...
    def get_action(self, obs):
        ...
        obs = obs.float()
        #print(self.ac.pi._distribution(obs).sample())
        #action = np.array(np.random.choice(np.array([0, 1, 2, 3]), p = self.ac.pi._distribution(obs)))
